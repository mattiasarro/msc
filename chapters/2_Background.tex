...
A simple yet surprisingly powerful way to represent text needs to average the word embedding contained in it \cite{}.
...
These kinds of models are good at  disambiguating the potentially many meanings a word might have,  and exactly what gets persisted in the final  vector representation of text depends on how the model is trained.
Two RNNs  trained on different tasks (such as sentiment analysis and named entity recognition)  would need to store different information in its hidden state to successfully  perform their relevant tasks,  hence the encoding for the same sentence would be different for either model.
