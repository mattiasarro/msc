...
Deep \textbf{autoencoders} can be used to learn low-dimensional representations of inputs.
...
If the bottleneck layer is unconstrained, it will use a wide range of values to represent of different inputs.
However, we would like embedding for similar inputs to also be similar. Variational autoencoders (\textbf{VAEs}) impose a prior distribution on the values of the embedding, often a multivariate Gaussian distribution with a diagonal covariance matrix, and the model is regularised during training to respect this prior.
In practice this means that  there is an additional step to determining the embedding $z$ of the input.
