...
These models will might become part of an ensemble, where diversity matters much more than the performance of any individual model.
...
We follow the notational trick where the first [column] of \textit{X} is always 1, and the first [column] of \textit{W} corresponds to the bias term.
...
Similarly to logistic regression, we have two cases: sigmoid activation for independent categories and softmax activation for mutually exclusive categories.
...
It is simply a model where a deep network and a logistic regression are trained jointly in the same SGD learning process, rather than trained separately and then ensembled.
...
For our task - both independent and mutually exclusive classes - the loss function binary cross-entropy between the training data and the model distribution, with the loss value averaged across all classes.
