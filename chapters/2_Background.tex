...
Logistic regression is a linear, discriminative classifier that is often the baseline model of choice, since it is a very interpretable model that is efficient to train.
...
p(y|x,w) = Ber(y|sigm(wTx))
 ...
 multiclass logistic regression, multinomial logistic regression,  maximum entropy classifier:

 maximum likelihood estimation log likelihood loss function for logistic regression is the binary cross entropy, which is described in section \ref{loss}
...
\label{loss}
 ...
 [Gradient-based models] such as logistic regression and neural networks are optimised with some variant of gradient descent.
  the loss function in both cases is binary cross entropy

 note that we are using the sum of log likelihoods, since products of non-log  likelihoods  would result in very small numbers that cannot be efficiently and accurately represented in digital computers.
 this is especially true if there are many probabilities multiplied together, which is often the case in both linear and deep models seen in academia and industry.
