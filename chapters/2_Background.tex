...
q:
committee methods take a simple and weighted average of the predictions from each model, essentially giving equal probability to each model
 the bIC  gives weight to each model depending on how well it fits and how many parameters it uses
  this is of  bayesian model.  the frequency version learns   a   weighting that that minimises the error on the population, which can be approximated on the  training set.  but sometimes this does not work well for example if the linear regression would put all of the weight of the largest model because it has the best protection accuracy.  the problem is that we have put all the models on an equal footing.
   stacked generalisations is a way of avoiding this, where the weights are  optimised   with K  fold cross validation,  better results can be obtained by restricting the weights to be nonnegative and to some is one
