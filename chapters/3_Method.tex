...
A group of tasks that would need to be run together repeatedly is encapsulated in an Airflow pipeline.
Figure \ref{af}
...
\begin{minipage}{.5\textwidth}
  ...
  \captionof{figure}{Airflow DAG: preprocess, train specific model, predict}
  \label{fig:af1}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  \includegraphics[width=.6\linewidth]{figures/af_train}
  \captionof{figure}{Airflow sub-DAG: train}
  \label{fig:af2}
\end{minipage}
\end{figure}
...
The per-category image embeddings were merged (using \textit{reduce}) into matrices of image embeddings in a given category, and the top 100 approximate nearest neighbours were extracted for each product using nmslib \cite{nmslib}.
...
To allow using the exact same program in three different settings - locally, inside a Docker container, and inside a MLE job - there is a prefix argument that specifies the directory or GCS bucket where the data is located.
...
In general, the trainer program would use tf.data.Dataset functions to build an input function for either training or evaluating, which would be supply data points for the train loop.
