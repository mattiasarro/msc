...
The training loss function of linear and deep models with all features (but no extra embeddings extracted with another pre-trained model) is shown in figure \ref{train_loss_all}.
...
The PR AUC on the final step (300k, or 220k for inceptionv3) is shown in figure \ref{pr_auc_chart}; recall on the final time step was omitted, as it was very similar to the final PR AUC scores.
...
\begin{figure}
  ...
  \caption{Hyperparameter ranges and the values found during automatic tuning.}
  \label{tuning_rounds}
\end{figure}
